...
# 문제 1. - 2018년 1월 매출의 총합
SELECT SUM(`sales`) AS `2018년 1월 매출 총합`
FORM `SALE`
WHERE `year`=2018 AND `month`=1 

# 문제 2. - 2019년 2월 5만원 이상 매출의 총합과 평균
SELECT
		SUM(`sales`) AS `총합`,
		AVG(`sales`) AS `평균`
FROM 
...
# 문제 3. - 2020년 전체 매출 가운데 최소, 최대 매출 조회
SELECT 
	MIN(`sales`) AS `최저`,
	MAX(`sales`) AS `최고`
FROM 
...
WHERE `sales` >= 50000
GROUP BY `uid`, `year`
HAVING `합계` >= 200000 #결과에서 20만원 이상만 출력
ORDER BY `합계` DESC;
...
FROM `SALE` AS a
JOIN `MEMBER` AS b
ON a.uid = b.uid; #line 200-202와 같음

SELECT a.seq, a.uid, a.sales, b.name, b.pos
FROM `SALE` AS a
JOIN `MEMBER` AS b
ON a.uid = b.uid; #필요한 data만 선별하기
